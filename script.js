...
const api_key = '6a622d203d720767ae95a14eb8efc6cf';
...
visibiltyVal =document.getElementById('visibiltyVal');
...
function getWeatherDetails(name, lat, lon, country, state) {
    ...
    fetch(AIR_POLLUTION_API_URL).then(res => res.json()).then(data => {
        ...
        alert('Failed to fetch Air Quality Index data');
    });
    ...
    fetch(WEATHER_API_URL).then(res => res.json()).then(data => {
        ...
        <div class="current-weather">
            ...
            visibiltyVal.innerHTML = `${(visibilty / 1000)} km`;
    ...
    fetch(FORECAST_API_URL).then(res => res.json()).then(data => {
        ...
        for (let i = 0; i < hourlyForecast.length; i++) {
            let date = new Date(hourlyForecast[i].dt_txt);
...
function getCityCoordinates() {
    ...
    fetch(GEOCODING_API_URL).then(res => res.json()).then(data => {
        ...
        alert(`Failed to fetch city coordinates of ${cityName}`);
    });
}
